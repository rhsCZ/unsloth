...
def _marker_inside_leading_envelope(content: str, enabled_tool_names: Optional[set] = None) -> bool:
    ...
    for _pat, (token, opener) in zip(_OUTER_ENVELOPE_CLOSED_PATS, required_pairs):
        ...
        if first_opener < 0:
            continue
        first_close = content.find(token)
        if first_close < 0:
            continue
        probe_start = first_close + len(token)
        ...
        m = _pat.search(content, 0, scan_end)
    ...
    for _pat, (token, opener) in zip(_OUTER_ENVELOPE_CLOSED_PATS, required_pairs):
        ...
        if first_opener < 0:
            continue
        first_close = residue.find(token)
        if first_close < 0:
            continue
        probe_start = first_close + len(token)
        needs_bound = (
        ...
        if not needs_bound:
            ...
            scan_end = residue.rfind(token) + len(token)
