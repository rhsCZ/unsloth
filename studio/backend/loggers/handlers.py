...
class LoggingMiddleware:
    ...
    def __init__(self, app: ASGIApp) -> None:
        self.app = app
        # (method, path, status_code) -> monotonic ts of the last EMITTED log.
        self._last_log: dict[tuple[str, str, int], float] = {}

    def _is_redundant_repeat(self, method: str, path: str, status_code: int, now: float) -> bool:
        """True if an identical GET/2xx log fired < window ago. Mutations and
        non-2xx are never deduped. Quiet-poll paths use a longer heartbeat
        window. Stamps only on emit, so steady polls still log periodically."""
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        try:
            ...
            if not excluded and not self._is_redundant_repeat(
                scope["method"], path, status_code, end_time
            ):
                logger.info(
                    "request_completed",
                    method = scope["method"],
