...
class TestGgufChatHistoryAlternation:
    def test_empty_assistant_turn_dropped_then_users_coalesced(self):
        ...
        sentinel) is dropped, and the orphaned user turns are coalesced so the
        GGUF chat path ships a strictly alternating history."""
        req = ChatCompletionRequest(
            model = "default",
            messages = [
                ChatMessage(role = "user", content = "hi"),
    ...
    def test_tool_path_rebuild_stays_alternating(self):
        """The GGUF tool path injects a system nudge with
        _set_or_prepend_system_message over the already-normalized history. Since
        the history is now coalesced, the tool path is strictly alternating too."""
        req = ChatCompletionRequest(
            model = "default",
            messages = [
                ChatMessage(role = "user", content = "hi"),
