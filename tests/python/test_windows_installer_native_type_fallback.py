...
# install.ps1 only, and that is the change rather than an oversight.
#
# studio/setup.ps1 used to carry a duplicate of this entire apparatus -- the Device Guard gate, the
# child-process probe, the dynamic assembly, the type emitter -- and its ONLY consumer was a cosmetic
# ANSI colour thunk. A CI pre-flight measured Windows PowerShell 5.1 attached to a real console and
# found the console mode already 0x7 before anything of ours ran: bit 0x4,
# ENABLE_VIRTUAL_TERMINAL_PROCESSING, was set by the host at startup, so the SetConsoleMode was
# re-setting a bit that was already set. The thunk became a read of $Host.UI.SupportsVirtualTerminal
# and ~290 lines went with it.
#
# setup.ps1 now declares no native imports at all, which test_setup_declares_no_native_imports_at_all
# pins so the deletion cannot quietly come back.
EMIT_SCRIPTS = ["install"]
