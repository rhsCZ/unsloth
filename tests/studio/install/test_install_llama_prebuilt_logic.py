...
def test_validate_prebuilt_choice_validation_disabled_by_default(tmp_path, monkeypatch):
    # The staged runtime smoke test is gated off (_RUN_STAGED_PREBUILT_VALIDATION is
    # False), so neither an approved nor an unapproved bundle runs quantize/server.
    approved = _run_validate_prebuilt_choice(monkeypatch, tmp_path, expected_sha256 = "ab" * 32)
    assert approved == {"quantize": 0, "server": 0}
    unapproved = _run_validate_prebuilt_choice(monkeypatch, tmp_path, expected_sha256 = None)
    assert unapproved == {"quantize": 0, "server": 0}


def test_validate_prebuilt_choice_runs_validation_when_flag_enabled(tmp_path, monkeypatch):
    # Flipping _RUN_STAGED_PREBUILT_VALIDATION back on restores the full smoke test,
    # proving the check and its fallback path are kept intact, only gated off.
