...
CLASSIFY_CASES: list[ClassifyCase] = [
    # Bog-standard shapes
    ClassifyCase("U01", "single-line static import",
                 "next-themes", "src/x.tsx",
                 'import { ThemeProvider } from "next-themes";',
                 "static_import"),
    ClassifyCase("U02", "side-effect import",
                 "katex", "src/x.tsx",
                 'import "katex/dist/katex.min.css";',
                 "side_effect_import"),
    ClassifyCase("U03", "dynamic import",
                 "@tauri-apps/api", "src/x.tsx",
                 'const { x } = await import("@tauri-apps/api/window");',
                 "dynamic_import"),
    ClassifyCase("U04", "require()",
                 "lodash", "src/x.js",
                 'const _ = require("lodash");',
                 "require"),
    ClassifyCase("U05", "CSS @import",
                 "tailwindcss", "src/x.css",
                 '@import "tailwindcss";',
                 "css_import"),
    # Sneaky shapes
    ClassifyCase("U06", "multi-line static import",
                 "next-themes", "src/x.tsx",
                 'import {\n  ThemeProvider,\n  useTheme,\n} from "next-themes";',
                 "static_import"),
    ClassifyCase("U07", "import type",
                 "@huggingface/hub", "src/x.ts",
                 'import type { PipelineType } from "@huggingface/hub";',
                 "static_import"),
    ClassifyCase("U08", "export * from re-export",
                 "@some-org/secrets", "src/x.ts",
                 'export * from "@some-org/secrets";',
                 "re_export"),
    ClassifyCase("U09", "export { x } from re-export",
                 "lodash-es", "src/x.ts",
                 'export { foo, bar } from "lodash-es";',
                 "re_export"),
    ClassifyCase("U10", "export type ... from re-export",
                 "@huggingface/hub", "src/x.ts",
                 'export type { Foo } from "@huggingface/hub";',
                 "re_export"),
    ClassifyCase("U11", "multi-line export from re-export",
                 "@some/pkg", "src/x.ts",
                 'export {\n  thing,\n  other,\n} from "@some/pkg";',
                 "re_export"),
    ClassifyCase("U12", "JSDoc @import",
                 "react", "src/x.ts",
                 '/** @type {import("react").FC} */\nconst Foo = () => null;',
                 "dynamic_import"),
    ClassifyCase("U13", "template literal package path",
                 "@assistant-ui/react", "src/x.tsx",
                 'const url = `@assistant-ui/react`;',
                 "template_literal"),
    ClassifyCase("U14", "new URL import-meta",
                 "monaco-editor", "src/x.ts",
                 'new URL("monaco-editor/esm/vs/editor/editor.worker", import.meta.url);',
                 "new_url"),
    ClassifyCase("U15", "tsc triple-slash type ref",
                 "@types/some-pkg", "src/x.ts",
                 '/// <reference types="@types/some-pkg" />',
                 "tsc_triple_slash"),
    ClassifyCase("U16", "HTML script src",
                 "alpinejs", "index.html",
                 '<script src="/node_modules/alpinejs/dist/cdn.min.js"></script>',
                 "html_script"),
    ClassifyCase("U17", "HTML link href",
                 "alpinejs", "index.html",
                 '<link rel="stylesheet" href="/node_modules/alpinejs/dist/style.css">',
                 "html_link"),
    ClassifyCase("U18", "bare quoted string in tsconfig paths",
                 "@huggingface/hub", "tsconfig.json",
                 '"paths": { "hf": ["@huggingface/hub/*"] }',
                 "string_literal"),
    ClassifyCase("U19", "vite alias key",
                 "@dagrejs/dagre", "vite.config.ts",
                 '"@dagrejs/dagre": path.resolve(__dirname, "./..."),',
                 "string_literal"),
    # False-positive guards (these should NOT detect)
    ClassifyCase("U20", "different package with shared prefix",
                 "foo", "src/x.ts",
                 'import { x } from "foobar";',
                 None),
    ClassifyCase("U21", "package mentioned in plain comment text",
                 "react", "src/x.ts",
                 '// We migrated from react-router to tanstack-router',
                 None),
    ClassifyCase("U22", "package name as a URL path tail is NOT detected "
                "(boundary rule: pkg must be followed by quote or `/`)",
                 "react", "src/x.ts",
                 'const docs = "https://example.com/react";',
                 None),
    ClassifyCase("U23", "package name in Python file (ignored, "
                "Python can never import npm packages)",
                 "playwright", "tests/x.py",
                 'label: str = "playwright"',
                 None),
    ClassifyCase("U24", "exact-prefix collision: pkg 'lodash' and 'lodash-es'",
                 "lodash", "src/x.ts",
                 'import _ from "lodash-es";',
                 None),
    ClassifyCase("U25", "scoped pkg substring collision",
                 "@radix-ui/react-label", "src/x.ts",
                 'import x from "@radix-ui/react-label-extra";',
                 None),
    ClassifyCase("U26", "package only mentioned in a markdown link",
                 "react", "README.md",
                 "See [react](https://react.dev).",
                 None),
    ClassifyCase("U27", "side-effect import with subpath",
                 "katex", "src/x.css",
                 '@import "katex/dist/katex.min.css";',
                 "css_import"),
    ClassifyCase("U28", "require.resolve",
                 "lodash", "build/x.cjs",
...
ADV_CASES: list[AdvCase] = [
    AdvCase("A01", "multi-line import of removed pkg should FAIL",
            "adv01.ts",
            'import {\n  foo,\n  bar,\n} from "__adv_only_pkg_a__";\n',
            "__adv_only_pkg_a__",
            "FAIL", ["__adv_only_pkg_a__"]),
    AdvCase("A02", "export * from removed pkg should FAIL",
            "adv02.ts",
            'export * from "__adv_only_pkg_b__";\n',
            "__adv_only_pkg_b__",
            "FAIL", ["__adv_only_pkg_b__"]),
    AdvCase("A03", "export { x } from removed pkg should FAIL",
            "adv03.ts",
            'export { foo, bar } from "__adv_only_pkg_c__";\n',
            "__adv_only_pkg_c__",
            "FAIL", ["__adv_only_pkg_c__"]),
    AdvCase("A04", "export type ... from removed pkg should FAIL",
            "adv04.ts",
            'export type { Foo } from "__adv_only_pkg_d__";\n',
            "__adv_only_pkg_d__",
            "FAIL", ["__adv_only_pkg_d__"]),
    AdvCase("A05", "package with similar prefix should NOT trigger FAIL",
            "adv05.ts",
            ...
            "__adv_only_pkg_e__",
            "PASS", []),
    AdvCase("A06", "dynamic import of removed pkg should FAIL",
            "adv06.ts",
            'const m = await import("__adv_only_pkg_f__");\n',
            "__adv_only_pkg_f__",
            "FAIL", ["__adv_only_pkg_f__"]),
    AdvCase("A07", "new URL of removed pkg should FAIL",
            "adv07.ts",
            'const w = new URL("__adv_only_pkg_g__/worker.js", import.meta.url);\n',
            "__adv_only_pkg_g__",
            "FAIL", ["__adv_only_pkg_g__"]),
    AdvCase("A08", "string-concat dynamic import is unanalyzable (PASS)",
            "adv08.ts",
            'const m = await import("__adv_only_" + "pkg_h__");\n',
            "__adv_only_pkg_h__",
            "PASS", []),
    AdvCase("A09", "package referenced only inside a JS comment "
            ...
            "__adv_only_pkg_i__",
            "FAIL", ["__adv_only_pkg_i__"]),
    AdvCase("A10", "package referenced only in a Python file should "
            "NOT trigger a JS FAIL",
            "adv10.py",
            'label = "__adv_only_pkg_j__"\n',
            "__adv_only_pkg_j__",
            "PASS", []),
    AdvCase("A11", "package mentioned in a markdown doc file is "
            "ignored by JS-like-only string_literal",
            "adv11.md",
            'See [docs](https://example.com/__adv_only_pkg_k__).\n',
            "__adv_only_pkg_k__",
            "PASS", []),
    AdvCase("A12", "JSDoc @import of removed pkg should FAIL",
            "adv12.ts",
            '/** @type {import("__adv_only_pkg_l__").Foo} */\n'
            'const x = null;\n',
            "__adv_only_pkg_l__",
            "FAIL", ["__adv_only_pkg_l__"]),
]
...
def run_adversarial_cases() -> int:
    ...
    for ac in ADV_CASES:
        ...
        try:
            ...
            ok = (actual_status == ac.expected_status
                  and set(fails) == set(ac.expected_failures))
            mark = "PASS" if ok else "FAIL"
